...
1. 0.05는 30 에폭 뒤 정확도 약 0.93(느림), 0.5는 약 0.96, 5는 loss가 `nan`(발산). 스케줄 없이 5로 두면 첫 에폭에서 터집니다.
2. `W2 (128×64), b2, W3 (64×10), b3`를 추가하고 forward에 `z2, h2`를, backward에 `dz2 = dh2 * (z2 > 0)`, `dW2 = h1.T @ dz2` 단계를 끼워 넣습니다. 수치 미분 오차가 1e-6 이하로 나오면 성공. 정확도는 32 은닉층과 비슷합니다(이 데이터는 작아서).
3. `h = np.tanh(z1)`, `dz1 = dh * (1 - h ** 2)`. 수치 미분 검사가 통과해야 합니다. 정확도는 ReLU와 비슷하지만 큰 학습률에서 더 안정적입니다(출력이 −1~1로 제한되므로).
4. 모멘텀은 같은 방향의 기울기를 누적해 가속하므로 초반 수렴이 빨라집니다(5 에폭에서 이미 0.95 근처). 단 `lr`은 조금 줄여야 할 수 있습니다.
5. 학습 후 `np.abs(W1).mean()`을 정규화 있을 때/없을 때 비교하면 있을 때가 작습니다. 정확도는 거의 같습니다(이 데이터는 과적합이 심하지 않음). 벌점의 미분이 `2·λ·W`인 것을 수치 미분으로 확인하세요.
6. `att = softmax(S)`, `out = att @ V`일 때 `dV = att.T @ dout`, `datt = dout @ V.T`, `dS = att * (datt − (datt * att).sum(-1, keepdims=True))`(softmax의 미분), `dQ = dS @ K / √d`, `dK = dS.T @ Q / √d`. 수치 미분으로 검증하세요.

...
1. 1 에폭: 말투가 절반쯤만 바뀌고 "냥"이 가끔 빠집니다. 15 에폭: 학습 질문에는 외운 답을 토씨까지 그대로, 새 질문에도 학습 데이터의 문장을 끌어다 씁니다(과적합). 이 데이터 크기에서는 3~5 에폭이 적당합니다.
2. r=1: 학습 파라미터 약 13만, 그래도 말투는 바뀝니다(말투는 저차원 변화). r=64: 약 860만, 결과는 r=8과 큰 차이 없음. 과제가 단순하면 r은 작아도 됩니다.
3. 질문 부분도 학습하면 loss 값이 달라지고(질문 토큰이 채점에 포함), 모델이 질문을 "생성"하는 것도 배웁니다. 결과 말투에는 큰 차이가 없지만, 학습 신호가 답이 아닌 부분에 분산되어 데이터가 적을 때 효율이 떨어집니다.
4. 20~50쌍이면 말투는 충분히 바뀝니다. 실패한다면 데이터의 말투가 서로 일관되지 않은 경우가 대부분입니다. 예시를 모두 소리 내어 읽어 보세요.
5. MLP 층을 추가하면 학습 파라미터가 약 3배(300만 정도)로 늘고, 같은 에폭에서 loss가 더 빨리 내려갑니다. 말투 과제에서는 눈에 띄는 차이가 없지만 지식·형식이 복잡한 과제에서는 도움이 됩니다.
6. fp16으로 불러 학습하면 loss가 `nan`이 되거나 학습이 안 되는 경우가 있습니다(fp16의 좁은 범위). bf16을 지원하는 GPU면 `torch.bfloat16`이 안전하고, T4에서는 fp32 + 작은 배치나 QLoRA가 현실적입니다.

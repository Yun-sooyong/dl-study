...
1. `["indoor", "outdoor"]`는 거의 다 맞힙니다. `["happy", "sad"]`처럼 추상적인 라벨은 표정이 뚜렷한 사진에서만 맞고, 나머지는 반반입니다. 시각적으로 드러나는 개념일수록 제로샷이 잘 됩니다.
2. 단어만 넣으면 확률이 덜 확신하고(최고 확률이 0.9 → 0.6대) 몇 장은 라벨이 바뀝니다. 문장 틀이 학습 분포에 가까워 성능이 좋습니다.
3. 이 CLIP은 영어 웹 데이터로 학습되어 한글 토큰을 거의 본 적이 없습니다. 검색 결과가 무작위에 가깝습니다. 다국어 CLIP(예: `M-CLIP`, `SigLIP` 다국어 버전)을 쓰거나 질의를 영어로 번역해야 합니다.
4. `Image.open` → `embed_images` → `image_vecs`에 이어 붙이면 됩니다. 50장 정도면 "노을", "고양이", "생일" 같은 질의가 잘 통합니다.
5. `img = embed_images(...)`, `txt = embed_texts(captions)`, `(img @ txt.T).argmax(1) == torch.arange(200)`의 평균. 캡션이 서로 비슷한 사진(개 사진 수십 장)이 많아 top-1은 0.3~0.5 정도로 낮게 나옵니다. top-5로 재면 0.7 이상입니다.

...
1. 0.3: 흔한 단어·형식만 반복해 지루하지만 철자가 정확합니다. 1.5: 다양하지만 없는 단어와 깨진 문장이 늘어납니다.
2. val loss가 1.5 근처까지 내려갑니다(작은 모델은 1.7 근처). 격차(train − val)도 커집니다. 데이터가 100만 글자뿐이라 큰 모델은 외우기 시작합니다. 더 돌리면 val이 다시 오릅니다.
3. train loss가 0.1 이하로 급락합니다. 각 위치가 정답(다음 글자)을 직접 보고 베끼기 때문입니다. 생성은 처음 몇 글자 뒤로 완전히 무의미합니다. "loss가 낮은데 결과가 나쁘면 누출을 의심하라"의 교과서적 예입니다.
4. loss가 2.5 근처에서 정체합니다. 어텐션이 순서를 모르므로 "직전 글자"라는 가장 중요한 정보를 못 씁니다. 생성 결과는 글자 통계만 맞는 뒤죽박죽입니다.
5. 한국어는 글자 종류(어휘)가 1000개 이상이라 초기 loss가 7 근처에서 시작하고, 같은 스텝 수로는 셰익스피어만큼 내려가지 않습니다. 임베딩 크기와 스텝 수를 늘려야 합니다. 조사가 붙은 그럴듯한 어절이 나오기 시작하면 성공입니다.
6. `att = ...` 네 줄을 `out = F.scaled_dot_product_attention(q, k, v, is_causal=True)`로. `mask` 버퍼가 필요 없어집니다. 결과는 같고 GPU에서 더 빠릅니다. [진짜에 가깝게](#32-scaling)의 코드가 이것입니다.

...
- 에폭마다 `evaluate`를 부르는 이유 — 학습이 진행되며 성능이 어떻게 변하는지 봐야 "얼마나 돌려야 하는지", "과적합이 시작됐는지"를 알 수 있습니다. [학습 잘 시키는 법](#24-training-recipes)에서 이것을 그래프로 발전시킵니다.
...
1. hidden=16: 파라미터 약 1.3만, 정확도 약 0.94. hidden=512: 약 40만, 약 0.98. 크면 좋아지지만 16배 파라미터에 정확도는 4%p. 수확 체감입니다.
2. ReLU가 없으면 두 Linear가 하나로 합쳐져 로지스틱 회귀와 같아지고, 정확도는 약 0.92(1부의 로지스틱 회귀 수준)에 머뭅니다.
3. `nn.Linear(784, 128), nn.ReLU(), nn.Linear(128, 64), nn.ReLU(), nn.Linear(64, 10)`. 앞 층의 출력 수 = 다음 층의 입력 수. 정확도는 비슷하거나 약간 오릅니다(3 에폭에서는 차이가 작음).
4. `lr=1e-1`: Adam에는 너무 커서 loss가 2.3 근처에서 요동치거나 발산합니다. `lr=1e-5`: 3 에폭 뒤에도 정확도 0.8대. 학습률은 10배 단위로 탐색합니다.
5. 곡선이 초반에 급격히 떨어지고 완만해지는 모양. 배치 하나의 loss라 요동이 심하므로 20~50개 이동평균을 그리면 읽기 쉽습니다.
